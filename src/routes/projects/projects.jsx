...
const ProjectsPage = () => {
  ...
  const clickCloseCreateProject = () => {
    ...
    // clear tasks and stop watching them
    setTasks((prev) => {
      for (let t in prev) {
        let taskRef = child(db, `tasks/${prev[t].uid}`);
        off(taskRef)
      }
      return []
    })
            ...
            <div className="items-list tasks-list">
              {tasks?.map((item, i) => (
                <Task
                  ...
                  editTask={editTask}
                  showConfirmDeleteTaskModal={showConfirmDeleteTaskModal}
                />
              ))}
            </div>
          )}
      ...
      {view === "share-project" && (
        <div className="content">
          <div className="mobile-buttons">
            <svg
              className={`cross ${view}`}
              onClick={() => clickCloseShareProject()}
              viewBox="0 0 100 100"
              width="40"
              height="40"
              stroke="white"
